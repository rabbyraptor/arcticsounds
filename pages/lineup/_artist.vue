    ...
    <div class="artist-content" v-if="this.artistInfo">
      <div class="left-side">
        ...
        <div class="artist-image" >
            <img :src="artistImage" />
            <span class="artist-photo-credit" v-if="this.artistInfo.photoCredit">Photo: {{ this.artistInfo.photoCredit }}</span>
        </div>
        <h2>{{this.artistInfo.title}} ({{ this.artistInfo.country }})</h2>
        <h3>{{this.artistInfo.style}}</h3>
        <p v-html="this.artistInfo.description" />
      </div>
      <div class="right-side">
        <div class="artist-links">
          <a class="artist-link" v-for="link in artistLinks.links" :key="link.title" :href="link.url" target="_blank">
            <svg
              class="svg-icon"
        ...
        <div class="spotify-embed" v-if="artistLinks.spotifyLink[0]">
          <iframe
            width="100%"
            height="300px"
            frameborder="0"
            :src="'//embed.spotify.com/?uri=' + artistLinks.spotifyLink[0].url"
  ...
  computed: {
    ...
    artistLinks() {
      ...
      for (let i in links) {
        if (links[i] != null) {
          if (
            links[i].split("|")[0] == "Spotify" ||
            links[i].split("|")[0] == "Top Tracks"
          ) {
            spotifyLink.push({
              title: "Spotify",
              url: links[i].split("|")[1]
            });
          } else if (links[i].split("|")[1] == null) {
            spotifyLink.push({
              title: "Spotify",
              url: links[i].split("|")[0]
            });
      ...
      return { links: newLinks, spotifyLink: spotifyLink };
    }
  }
};
</script>
